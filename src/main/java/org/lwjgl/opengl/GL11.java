...
public class GL11 extends RealOpenGLEnums {
	...
	public static void glCopyTexSubImage2D(int target, int level, int dx, int dy, int sx, int sy, int w, int h) {
	...
	public static void glTexGeni(int coord, int pname, int param) {
		if (pname == GL_TEXTURE_GEN_MODE) {
			GlStateManager.texGen(mapTexGenCoord(coord), param);
		}
	}

	public static void glTexGen(int coord, int pname, FloatBuffer params) {
		GlStateManager.func_179105_a(mapTexGenCoord(coord), pname, params);
	}

	private static GlStateManager.TexGen mapTexGenCoord(int coord) {
		switch (coord) {
		case GL_S:
		...
		default:
			throw new IllegalArgumentException("Invalid texgen coord: " + coord);
		}
	}
}
