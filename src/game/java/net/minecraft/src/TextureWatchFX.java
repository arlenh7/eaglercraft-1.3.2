...
public class TextureWatchFX extends TextureFX
{
    ...
    public TextureWatchFX(Minecraft par1Minecraft)
    {
        ...
        if (var2 != null)
        {
            ...
            ImageData sub = var2.getSubImage(var3, var4, 16, 16);
            System.arraycopy(sub.pixels, 0, this.watchIconImageData, 0, this.watchIconImageData.length);
        }

        ImageData var5 = ImageData.loadImageFile(EagRuntime.getResourceStream("/misc/dial.png"));

        if (var5 != null)
        {
            var5 = var5.swapRB();
            ImageData subDial = var5.getSubImage(0, 0, 16, 16);
            System.arraycopy(subDial.pixels, 0, this.dialImageData, 0, this.dialImageData.length);
        }
    }
