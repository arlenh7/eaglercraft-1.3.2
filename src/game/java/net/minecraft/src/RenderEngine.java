...
public class RenderEngine
{
    ...
    /** A mapping from GL texture names (integers) to ImageData instances */
    ...
    private ImageData missingTextureImage = createMissingTexture();
    ...
    private static ImageData createMissingTexture()
    {
        int size = 64;
        ImageData img = new ImageData(size, size, true);
        ...
        for (int y = 0; y < size; ++y)
        {
            ...
            {
                boolean even = ((x >> 3) + (y >> 3)) % 2 == 0;
                pixels[x + y * size] = even ? 0xFFFF00FF : 0xFF000000;
            }
        }
    ...
    private int[] getImageContentsAndAllocate(ImageData par1Image)
    {
        int var2 = par1Image.width;
        int var3 = par1Image.height;
        int[] var4 = new int[var2 * var3];
        System.arraycopy(par1Image.pixels, 0, var4, 0, var4.length);
        return var4;
    }

    private int[] getImageContents(ImageData par1Image, int[] par2ArrayOfInteger)
    {
        int var3 = par1Image.width;
        int var4 = par1Image.height;
        int len = Math.min(par2ArrayOfInteger.length, var3 * var4);
        System.arraycopy(par1Image.pixels, 0, par2ArrayOfInteger, 0, len);
     ...
     * stacked vertically
     */
    private ImageData unwrapImageByColumns(ImageData par1Image)
    {
        int columns = par1Image.width / 16;
        int srcWidth = par1Image.width;
        int srcHeight = par1Image.height;
        ImageData out = new ImageData(16, srcHeight * columns, par1Image.alpha);
        int[] src = par1Image.pixels;
        int[] dst = out.pixels;
        int dstWidth = out.width;

        for (int col = 0; col < columns; ++col)
        {
            int srcX = col * 16;
            int dstY = col * srcHeight;

            for (int y = 0; y < srcHeight; ++y)
            {
                System.arraycopy(src, y * srcWidth + srcX, dst, (dstY + y) * dstWidth, 16);
            }
        }
     ...
     */
    public int allocateAndSetupTexture(ImageData par1Image)
    {
        this.singleIntBuffer.clear();
        GLAllocation.generateTextureNames(this.singleIntBuffer);
        int var2 = this.singleIntBuffer.get(0);
        this.setupTexture(par1Image, var2);
        this.textureNameToImageMap.addKey(var2, par1Image);
        return var2;
    }
     ...
     */
    public void setupTexture(ImageData par1Image, int par2)
    {
    ...
    /**
     * Returns an ImageData read off the provided input stream. Args: inputStream
     */
    private ImageData readTextureImage(InputStream par1InputStream) throws IOException
    {
        ImageData var2 = ImageData.loadImageFile(par1InputStream);
        par1InputStream.close();
        return var2 != null ? var2.swapRB() : null;
    }
