	...
	public void initGui() {
		if (this.lines == null) {
			...
			try {
				...
				List<String> winLines = EagRuntime.getResourceLines("/title/win.txt");
				if (winLines != null) {
					for (int i = 0, l = winLines.size(); i < l; ++i) {
						var1 = winLines.get(i);
						String var7;
						String var8;

						for (var1 = var1.replaceAll("PLAYERNAME", this.mc.session.username); var1.contains(var2);
								var1 = var7 + "\u00a7f\u00a7k" + "XXXXXXXX".substring(0, var5.nextInt(4) + 3) + var8) {
				...
				List<String> creditLines = EagRuntime.getResourceLines("/title/credits.txt");
				if (creditLines != null) {
					for (int i = 0, l = creditLines.size(); i < l; ++i) {
						var1 = creditLines.get(i);
