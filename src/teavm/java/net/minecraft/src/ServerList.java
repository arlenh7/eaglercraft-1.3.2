...
public class ServerList
{
    ...
    public void loadServerList()
    {
        try
        {
            VFile2 serversFile = new VFile2(this.mc.mcDataDir, "servers.dat");
            if (!serversFile.exists())
            {
                return;
            }

            DataInputStream in = new DataInputStream(serversFile.getInputStream());
            NBTTagCompound var1;

            try
            {
                var1 = CompressedStreamTools.read(in);
            }
            finally
            {
                in.close();
    ...
    public void saveServerList()
    {
        try
        {
            ...
            VFile2 serversFile = new VFile2(this.mc.mcDataDir, "servers.dat");
            DataOutputStream out = new DataOutputStream(serversFile.getOutputStream());
            try
            {
                CompressedStreamTools.write(var5, out);
            }
            finally
            {
                out.close();
